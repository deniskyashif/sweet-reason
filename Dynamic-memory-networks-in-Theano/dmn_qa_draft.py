...
class DMN_qa:
    ...
    def __init__(self, babi_train_raw, babi_test_raw, word2vec, word_vector_size, 
        ...
        self.inp_c = T.stack([self.inp_c] * 2).transpose((1, 2, 0)) # (fact_cnt, dim, 4)
    ...
    def new_attention_step(self, ct, prev_g, mem, q_q, c_vecs):
        cWq = T.dot(T.ones((1, 2), dtype=floatX), T.dot(T.dot(ct.T, self.W_b), q_q) * T.eye(n=2, m=2, dtype=floatX))
        cWm = T.dot(T.ones((1, 2), dtype=floatX), T.dot(T.dot(ct.T, self.W_b), mem) * T.eye(n=2, m=2, dtype=floatX))
        cWc_vecs = T.dot(T.ones((1, 2), dtype=floatX), T.dot(T.dot(ct.T, self.W_b), c_vecs) * T.eye(n=2, m=2, dtype=floatX))
    ...
    def step(self, batch_index, mode):
        ...
        if mode == 'train':
            ...
            gradient_value = self.get_gradient_fn(inp, q, ans, ca, cb, input_mask)
