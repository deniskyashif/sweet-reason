...
def init_semeval(data_file):
    ...
    for instance in instances:
        ...
        for question in instance.questions:
            if all(answer.text.find(' ') == -1 for answer in question.answers):
                new_questions.append(question)
        instance.questions = new_questions;
        if len(instance.questions) > 0:
    ...
    for instance in instances:
        curr_task = {"C": [], "Q": "", "A": "", "A1":"", "A2":""}

        sent_text = instance.text.strip().replace('.',' . ' ).replace('!', ' . ')
        curr_task["C"]= sent_text

        for question in instance.questions:
            curr_task["Q"] = question.text.replace('?', '')
            curr_task["A1"] = filter(lambda a: a.id == "0", question.answers)[0].text
            curr_task["A2"] = filter(lambda a: a.id == "1", question.answers)[0].text
            curr_task["A"] = int(filter(lambda a: a.isCorrect, question.answers)[0].id)
